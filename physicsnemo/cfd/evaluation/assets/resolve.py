...
def resolve_model_assets(
    ...
    """
    Return local ``checkpoint_path``, ``stats_path``, and optional stable ``asset_identity`` for metrics cache.
    ...
    spec: AssetSpec | None = get_default_asset(model_config.name) if not pkg_root else None
    ...
    identity = f"package:{pkg_root}|{ck_rel}|{st_rel}"
    return ck_path, st_path, identity
