...
An ensemble is the canonical "real" counterpart to the throwaway MC weight-perturbation proxy
(:mod:`..mc_perturbation`): instead of jittering one model's weights, it aggregates the predictions
of several independently-saved model checkpoints. The engine drives it through the SAME
``UQ_METHOD="sampling"`` path — but here each "pass" is a genuine model, so the across-member spread
