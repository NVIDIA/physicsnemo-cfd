...
- `--stream-velocity FLOAT`: free-stream inlet velocity [m/s]. Default `38.889`.
- `--stencil-size INT`: number of k-NN neighbors for the surface stencil. Default `7`.
- `--air-density FLOAT`: free-stream air density [kg/m^3]. Default `1.205`.
- `--verbose` / `--no-verbose`: tqdm progress bar over inference batches. Default `--verbose`.
...
- `smooth_sensitivity_normal_cell`: (n_faces,) point-smoothed normal component 
  on cells
